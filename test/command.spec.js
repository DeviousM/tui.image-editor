...
describe("commandFactory", () => {
    ...
    describe("functions", () => {
        it("can register custom command", done => {
            ...
            spyOn(testCommand, "execute").and.returnValue(
                Promise.resolve("testCommand")
            );
    ...
    describe("loadImageCommand", () => {
        ...
        it("should load new image", done => {
            invoker
                ...
                .then(sizeChange => {
                    ...
                    expect(graphics.getCanvasImage().getSrc()).toContain(
                        imageURL
                    );
        ...
        it("After running the LOAD_IMAGE command, existing objects should not include cropzone.", done => {
            ...
            invoker
                .execute(commands.ADD_OBJECT, graphics, objCropzone)
                .then(() => {
                    invoker
                        .execute(
                            commands.LOAD_IMAGE,
                        ...
                        .then(() => {
                            ...
                            const savedObjects =
                                invoker._undoStack[lastUndoIndex].undoData
                                    .objects;
        ...
        it("`evented` attribute of the saved object must be true after LOAD_IMAGE.", done => {
            ...
            invoker
                .execute(commands.ADD_OBJECT, graphics, objCircle)
                .then(() => {
                    invoker
                        .execute(
                            commands.LOAD_IMAGE,
                        ...
                        .then(() => {
                            const lastUndoIndex = invoker._undoStack.length - 1;
                            const [savedObject] = invoker._undoStack[
                                lastUndoIndex
                            ].undoData.objects;
